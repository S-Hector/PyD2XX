...
if(Device.Flags & PyD2XX.FT_FLAGS_OPENED):
    ...
    print("  Device is free! Will attempt to get modem & line status!")
    Status = PyD2XX.FT_Open(0, Device)
    if(Status != PyD2XX.FT_OK):
        print(PyD2XX.FT_STATUS_STR[Status] + " | FAILED TO OPEN DEVICE: ABORTING")
        exit()
    print("  Successfully opened!")
    Status, ModemStatus = PyD2XX.FT_GetModemStatus(Device)
    if(Status != PyD2XX.FT_OK):
    ...
    print("  --| Line Status |--")
    print("  OE = " + str(bool((ModemStatus >> 8) & int("0x02", 16))))
    print("  PE = " + str(bool((ModemStatus >> 8) & int("0x04", 16))))
    print("  FE = " + str(bool((ModemStatus >> 8) & int("0x08", 16))))
    print("  BI = " + str(bool((ModemStatus >> 8) & int("0x10", 16))))
