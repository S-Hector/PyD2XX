...
if(Device.Flags & PyD2XX.FT_FLAGS_OPENED):
    ...
    print("  Device is free! Will attempt to clear RTS!")
    Status = PyD2XX.FT_Open(0, Device)
    if(Status != PyD2XX.FT_OK):
        print(PyD2XX.FT_STATUS_STR[Status] + " | FAILED TO OPEN DEVICE: ABORTING")
        exit()
    print("  Successfully opened!")
    Status = PyD2XX.FT_ClrRts(Device)
    if(Status != PyD2XX.FT_OK):
        print(PyD2XX.FT_STATUS_STR[Status] + " | FAILED TO SET DTR: ABORTING")
        exit()
    print("  Successfully cleared RTS!")
