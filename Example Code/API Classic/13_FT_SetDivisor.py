...
if(Device.Flags & PyD2XX.FT_FLAGS_OPENED):
    ...
    print("  Device is free! Will attempt to set divisor!")
    Status = PyD2XX.FT_Open(0, Device)
    if(Status != PyD2XX.FT_OK):
        print(PyD2XX.FT_STATUS_STR[Status] + " | FAILED TO OPEN DEVICE: ABORTING")
        exit()
    print("  Successfully opened!")
    Status = PyD2XX.FT_SetDivisor(Device, DIVISOR)
    if(Status != PyD2XX.FT_OK):
        print(PyD2XX.FT_STATUS_STR[Status] + " | FAILED TO SET DIVISOR: ABORTING")
        exit()
    print("  Successfully set divisor to " + str(DIVISOR) + ".")
