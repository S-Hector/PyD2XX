...
if(Device.Flags & PyD2XX.FT_FLAGS_OPENED):
    ...
    print("  Device is free! Will attempt to get driver version!")
    Status = PyD2XX.FT_Open(0, Device)
    if(Status != PyD2XX.FT_OK):
        print(PyD2XX.FT_STATUS_STR[Status] + " | FAILED TO OPEN DEVICE: ABORTING")
        exit()
    print("  Successfully opened!")
    Status, DriverVersion = PyD2XX.FT_GetDriverVersion(Device)
    if(Status != PyD2XX.FT_OK):
        print(PyD2XX.FT_STATUS_STR[Status] + " | FAILED TO GET DRIVER VERSION: ABORTING")
        exit()
